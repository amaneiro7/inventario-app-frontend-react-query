...
import type { ClearCacheParams } from '../domain/dto/ClearCache.dto'
...
export class CLearCacheUseCase {
	 ...
	 * @param clearCacheRepository - The repository responsible for resetting user passwords.
	 * @param events - The event manager to notify about the operation's progress (loading, success, error).
	 */
	constructor(
		private readonly clearCacheRepository: ClearCacheRepository,
	 ...
	 */
	async execute(params: ClearCacheParams) {
		try {
			this.events.notify({ type: 'loading', message: 'Procesando...' })

			return await this.clearCacheRepository.run(params).then(res => {
				this.events.notify({
					type: 'success',
