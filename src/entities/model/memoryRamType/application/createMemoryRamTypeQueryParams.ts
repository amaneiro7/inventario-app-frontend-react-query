...
/**
 * Defines the structure for filtering and pagination parameters when searching for memory RAM types.
 */
export interface MemoryRamTypeFilters {
	id?: MemoryRamTypeDto['id']
	name?: MemoryRamTypeDto['name']
	pageNumber?: number
	pageSize?: number
...
/**
 * Creates a query string for searching memory RAM types based on provided filters and pagination options.
 ...
 */
export async function createMemoryRamTypeParams({
	pageNumber,
	pageSize,
	orderBy,
	orderType,
	...options
}: MemoryRamTypeFilters): Promise<string> {
