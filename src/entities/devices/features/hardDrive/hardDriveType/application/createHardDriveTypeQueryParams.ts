...
/**
 * @interface HardDriveTypeFilters
 * @description Define la estructura de los filtros disponibles para buscar entidades `HardDriveType`.
 * @property {HardDriveTypeDto['id']} [id] - ID del tipo de disco duro.
 * @property {HardDriveTypeDto['name']} [name] - Nombre del tipo de disco duro.
...
export interface HardDriveTypeFilters {
	id?: HardDriveTypeDto['id']
	name?: HardDriveTypeDto['name']
	pageNumber?: number
	pageSize?: number
 ...
 * @function
 * @description Construye una cadena de parámetros de consulta (query string) a partir de un objeto `HardDriveTypeFilters`.
 * Utiliza la clase `Criteria` para generar la consulta de forma estructurada.
 * @param {HardDriveTypeFilters} filters - El objeto de filtros para construir los parámetros de consulta.
 * @returns {Promise<string>} Una promesa que se resuelve con la cadena de parámetros de consulta.
 */
export async function createHardDriveTypeParams({
	pageNumber,
	pageSize,
	orderBy,
	orderType,
	...options
}: HardDriveTypeFilters): Promise<string> {
