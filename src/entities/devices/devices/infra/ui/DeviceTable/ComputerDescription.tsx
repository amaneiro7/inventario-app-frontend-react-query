...
interface ComputerDescriptionProps {
	open: boolean
	device: DeviceDto
	colSpan: number
...
export const ComputerDescription = memo(
	({ open, device, colSpan, visibleColumns }: ComputerDescriptionProps) => {
		return (
			<>
				...
				>
					...
					{device.employee?.phone && device.employee?.phone.length > 0 && (
						<TableCellDescInfo
							title="Teléfono"
							text={device.employee?.phone
								?.map(tel => formatearTelefono(tel))
								.join('  ')}
						/>
					)}
					...
					<div className="md:grid md:grid-cols-2 md:grid-rows-2 md:gap-2">
						<TableCellDescInfo
							...
							text={
								device.computer ? `${device?.computer?.memoryRamCapacity} Gb` : ''
							}
						/>
						<TableCellDescInfo
							title="Modulos"
...
ComputerDescription.displayName = 'ComputerDescription'
