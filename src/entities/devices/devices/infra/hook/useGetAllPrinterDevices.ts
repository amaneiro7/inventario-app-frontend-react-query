...
/**
 * `useGetAllPrinterDevices`
 * @function
 * @description Hook personalizado para obtener todos los dispositivos de tipo 'impresora' o un subconjunto filtrado/paginado.
 * Utiliza `react-query` para la gestión de la caché y el estado de la petición, con refetching automático.
 * @param {DeviceBaseFilters} query - Objeto de filtros para la consulta de impresoras.
 ...
 * @property {import('@/entities/shared/domain/methods/Response').Response<import('../../domain/dto/Device.dto').DeviceDto> | undefined} devices - Los datos de los dispositivos de impresora obtenidos de la consulta.
 */
export const useGetAllPrinterDevices = (query: DeviceBaseFilters) => {
	const {
		isLoading,
		refetch,
		isError,
