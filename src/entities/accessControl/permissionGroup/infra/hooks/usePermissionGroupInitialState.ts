...
/**
 * `usePermissionGroupInitialState`
 * @function
 ...
 * @returns {{ initialState: DefaultPermissionGroup; resetState: () => void; mode: 'edit' | 'add' }}
 * Un objeto con el estado inicial de la marca, una función para resetear el estado y el modo actual del formulario.
 */
export function usePermissionGroupInitialState(defaultState: DefaultPermissionGroup): {
	initialState: DefaultPermissionGroup
	resetState: () => void
	mode: 'edit' | 'add'
	isLoading: boolean
	isNotFound: boolean
	...
	const [state, setState] = useState<DefaultPermissionGroup>(defaultState) // Estado local de la marca.
	...
	const {
		...
		enabled: !!id && mode === 'edit' && !location?.state?.permissionGroup, // Habilita la consulta solo si hay un ID, el modo es editar y no hay datos en el estado de la ubicación.
	 ...
	 */
	const mapPermissionGroupToState = useCallback((permissionGroup: PermissionGroupDto): void => {
	...
	useEffect(() => {
		...
		if (location?.state?.permissionGroup) {
			setState(location.state.permissionGroup)
		} else if (permissionGroupData) {
			// Si hay datos de la API, actualiza el estado con esos datos.
			mapPermissionGroupToState(permissionGroupData)
		}
	...
	const resetState = useCallback(async () => {
		...
		if (mode === 'add') {
			...
			if (data) {
				mapPermissionGroupToState(data)
			}
		}
