...
export class PermissionDescription extends StringValueObject {
	...
	/**
	 * Crea una instancia de `PermissionDescription`.
	 * @param {string} value - El valor del nombre de la marca.
	 * @throws {Error} Si el valor no es válido según las reglas definidas.
	 */ constructor(value: string) {
		super(value)
		if (!PermissionDescription.isValid(value)) {
			throw new Error(PermissionDescription.invalidMessage(value))
		}
	}
	 ...
	 */ public static isValid(value: string): boolean {
		return (
			value.length >= PermissionDescription.NAME_MIN_LENGTH &&
			value.length <= PermissionDescription.NAME_MAX_LENGTH
		)
	}
	 ...
	 */ public static invalidMessage(value: string): string {
		return `El nombre ${value} no es válido. Debe tener entre ${PermissionDescription.NAME_MIN_LENGTH} y ${PermissionDescription.NAME_MAX_LENGTH} caracteres`
	}
}
