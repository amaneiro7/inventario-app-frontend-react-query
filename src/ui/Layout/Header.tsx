...
const Button = lazy(async () => import("../../components/Button/Button").then(m => ({ default: m.default })))

const ConfirmationModal = lazy(async () => import("@/ui/Modal/ConfirmationModal").then(m => ({ default: m.ConfirmationModal })))
const Modal = lazy(async () => import('@/ui/Modal/Modal').then(m => ({ default: m.Dialog })))
        ...
        <>
            <header className='min-h-16 h-16 md:text-sm md:border-none gap-4 flex items-center justify-between md:top-0 md:sticky z-50 bg-azul w-full shadow-lg pr-8 py-4 overflow-visible'>
                ...
                <div className='flex flex-1 gap-8 items-center justify-end'>
                    ...
                    <Link to='/profile' className='text-white text-xs md:text-sm lg:text-base font-medium p-1 border-b hover:text-orange hover:border-orange transition-colors duration-200'>Perfil</Link>
