...
async function ChargeTables () {
    ...
    for (let i = 0; i < cases.length; i++) {
        ...
        if (asignado != "" && getName(element.AsignadoStringId).toLowerCase().indexOf(asignado.toLowerCase()) == -1) continue;
        if (cargado != "" && getName(element.UsuarioStringId).toLowerCase().indexOf(cargado.toLowerCase()) == -1) continue;
        ...
        */
        ...
        if (!isCAI) {
            if (isSecurity && (isSupport || isAS400 || isPerimetral || isPlataforma || isCompliace || isServidor || isSegAcc)) {
                if ((element.GroupsDepId != 3575) && (element.GroupsDepId != 3576) && (element.GroupsDepId != 3760) && (element.GroupsDepId != 3597) && (element.GroupsDepId != 3596) && (element.GroupsDepId != 3595) && (element.GroupsDepId != 3598) && (element.GroupsDepId != 3599)) continue;
            }
            if (isNotSecurity && isMasterSupp) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3576) continue;
            }
            if (isNotSecurity && isSupport && isNotMasterSupp) {//Si pertenece al 13, filtra por asignado
                if ((element.AsignadoId != _spPageContextInfo.userId)) continue;
            }
            if (isNotSecurity && isSegAcc) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3760) continue;
            }
            if (isNotSecurity && isAS400) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3597) continue;
            }
            if (isNotSecurity && isPerimetral) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3596) continue;
            }
            if (isNotSecurity && isPlataforma) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3595) continue;
            }
            if (isNotSecurity && isCompliace) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3598) continue;
            }
            if (isNotSecurity && isServidor) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3599) continue;
            }
            if (isSerAdm) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3609) continue;
            }
            if (isSerBan) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3610) continue;
            }
            if (isSerGen) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3611) continue;
            }
            if (isTele) {//Si pertenece al 13, filtra por asignado
                if (element.GroupsDepId != 3606) continue;
            }
            if (!isSupport && !isSecurity && !isAS400 && !isPerimetral && !isPlataforma && !isCompliace && !isServidor && !isSerAdm && !isSerBan && !isSerGen && !isTele) {
...
async function GetInfoCases (parentId) {
    ...
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: _spPageContextInfo.siteAbsoluteUrl + "/_api/lists/getbytitle('Caso Publico')/Items" + filter,
            type: "GET",
            headers: {
