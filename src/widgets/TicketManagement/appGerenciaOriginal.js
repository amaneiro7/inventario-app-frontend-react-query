...
async function ChargeTables() {
	...
	let tableHeader =
		'<table id=\'tblCasos\' class="table table-striped table-hover">' +
	...
	for (let i = 0; i < cases.length; i++) {
		...
		//console.log(element);
		if (codCaso != '' && element.Id != codCaso) continue
		if (prioridad != '' && element.Prioridad != prioridad) continue
		if (status != '' && element.Status != status) continue
		if (tCaso != '' && element.Tipo_x0020_de_x0020_CasoId != tCaso) continue
		if (Depa != '' && element.GroupsDepId != Depa) continue
		if ((pCaso == 'Si' && element.ParentCod) || (pCaso == 'No' && !element.ParentCod)) continue
		if (agencia != '' && element.Zona != agencia) continue
		if (
			asignado != '' &&
			getName(element.AsignadoStringId).toLowerCase().indexOf(asignado.toLowerCase()) == -1
		)
			continue
		if (
			cargado != '' &&
			getName(element.UsuarioStringId).toLowerCase().indexOf(cargado.toLowerCase()) == -1
		)
			continue
		let fecha = element.Fecha
		if (hasta != '' && desde != '' && (fecha <= desde + 1 || fecha >= hasta - 1)) continue

		/*if (status!=""&&element.Status!=status) continue;
        if (element.Asignado_x0020_aId!=null) continue;*/
		/*
            soporte que solo vea los asignados a el usuario del grupo 3576
            g de seguridad que solo vea sus cargodos por 3575
            cai que veo todo 3567
        ...
        */
		...
		let isNotMasterSupp = groupsId.indexOf(3843) == -1
		...
		if (!isCAI) {
			if (
				...
				if (
					element.GroupsDepId != 3575 &&
					element.GroupsDepId != 3576 &&
					element.GroupsDepId != 3760 &&
					element.GroupsDepId != 3597 &&
					element.GroupsDepId != 3596 &&
					element.GroupsDepId != 3595 &&
					element.GroupsDepId != 3598 &&
					element.GroupsDepId != 3599
				)
					continue
			}
			if (isNotSecurity && isMasterSupp) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3576) continue
			}
			if (isNotSecurity && isSupport && isNotMasterSupp) {
				//Si pertenece al 13, filtra por asignado
				if (element.AsignadoId != _spPageContextInfo.userId) continue
			}
			if (isNotSecurity && isSegAcc) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3760) continue
			}
			if (isNotSecurity && isAS400) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3597) continue
			}
			if (isNotSecurity && isPerimetral) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3596) continue
			}
			if (isNotSecurity && isPlataforma) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3595) continue
			}
			if (isNotSecurity && isCompliace) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3598) continue
			}
			if (isNotSecurity && isServidor) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3599) continue
			}
			if (isSerAdm) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3609) continue
			}
			if (isSerBan) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3610) continue
			}
			if (isSerGen) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3611) continue
			}
			if (isTele) {
				//Si pertenece al 13, filtra por asignado
				if (element.GroupsDepId != 3606) continue
			}
			if (
				!isSupport &&
				!isSecurity &&
				...
				if (_spPageContextInfo.userLoginName.indexOf(element.Title) == -1) continue //element.Login
			}
		}
		...
		textTablesContainer +=
			...
			'</td>' +
			'<td>' +
			(element.ParentCod || 'No es un caso remitido') +
			'</td>' +
			'<td>' +
			getName(element.UsuarioId) +
			'</td>' +
			'<td>' +
			getName(element.AsignadoId) +
			'</td>' +
			'<td>' +
			getGroupsName(element.GroupsDepId) +
			'</td>' +
			'<td>' +
			(element.Nombre || '') +
			'</td>' +
			...
			'</td>' +
			'<td>' +
			getTypeCaseName(element.Tipo_x0020_de_x0020_CasoId) +
			'</td>' +
			'<td>' +
			element.Prioridad +
			'</td>' +
			...
			'</td>' +
			'<td>' +
			formatDate(element.Fecha_x0020_Limite) +
			'</td>' +
			'<td>' +
			formatDate(element.FechaCulminacion) +
			'</td>' +
			'<td>' +
			truncarTexto(element.Descripcion, 25) +
			'</td>' +
			'<td>' +
			truncarTexto(element.Comentarios, 25) +
			'</td>' +
			'<td>' +
			truncarTexto(element.Observaciones, 25) +
			'</td>' +
			'<td>' +
			getEjecutado(element.EjecutadoId) +
			'</td>' +
			'</tr>'
...
async function GetInfoCases(parentId) {
	...
	if (!parentId) filter = `?$top=100000`
	return new Promise(function (resolve, reject) {
		$.ajax({
			url:
				_spPageContextInfo.siteAbsoluteUrl +
				"/_api/lists/getbytitle('Caso Publico')/Items" +
				filter,
			type: 'GET',
			headers: {
...
async function GetInfoUserCCFromDatabase(userName) {
	return new Promise(function (resolve, reject) {
		$.ajax({
			url:
				_spPageContextInfo.siteAbsoluteUrl +
				`/_api/lists/getbytitle('Directory')/Items?&$filter=Login eq '${userName}'`,
			type: 'GET',
			headers: {
				accept: 'application/json;odata=verbose',
				'content-type': 'application/json;odata=verbose',
...
async function GetInfoCase(Id) {
	return new Promise(function (resolve, reject) {
		$.ajax({
			...
			url:
				_spPageContextInfo.siteAbsoluteUrl +
				`/_api/lists/getbytitle('Caso Publico')/Items(${Id})`,
			type: 'GET',
			headers: {
				accept: 'application/json;odata=verbose',
				'content-type': 'application/json;odata=verbose',
...
async function GetInfoTypeCases() {
	return new Promise(function (resolve, reject) {
		$.ajax({
			url:
				_spPageContextInfo.siteAbsoluteUrl +
				"/_api/lists/getbytitle('Control de Casos')/Items",
			type: 'GET',
			headers: {
				accept: 'application/json;odata=verbose',
				'content-type': 'application/json;odata=verbose',
...
function CreateOrUpdateCase() {
	...
	if (form2 == '') {
		alert(
			'La información no ha cargado correctamente, espere un momento o contacte al departamento de correspondiente'
		)
		return
	}
...
async function ChargeUsersForm() {
	...
	usuarios = usuarios.filter(
		x =>
			['System Account', 'Miguelito', 'sp admin', 'sp apppool', 'SPSearch'].indexOf(
				x.Title
			) == -1
	)
	let isSupport = groupsId.indexOf(3576) != -1
...
async function Init() {
	...
	try {
		await ChargeTypeCases()
	} catch {}
	try {
		await ChargeUsersForm()
	} catch {}
	try {
		await LoadCase()
	} catch {}
